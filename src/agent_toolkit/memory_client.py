...
scoutmaster memory like donna") and removed 2026-09-04
(gretchen-workspace#10): `search_donna_scoutmaster_memory` used to hardcode
`agent="donna", project="scoutmaster"` for read-only access into Donna's
pre-split troop-knowledge partition. That partition was wiped outright
(19,151 documents forgotten) rather than migrated -- "Gretchen rebuilds her
own memory from here" -- so the read-through function had nothing left to
read and was removed along with its two call sites in donna-workspace
(general.py's search_scoutmaster tool, email_draft.py's grounding fallback).
